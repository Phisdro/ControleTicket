...
begin
  ...
  if not(Trim(edtFiltroNome.Text).IsEmpty) then
    DMControle.fdqRelatorio.ParamByName('NOME').AsString := Trim(edtFiltroNome.Text);
...
begin
    case MessageDlg('Atenção!'+#10+#10+'As alterações não salvas serão perdidas, deseja salvar?',
